...
x3=pd.DataFrame(x)
x3.columns=['text']
d=news_df['news_category']
x3=pd.concat([x3,d],axis=1)

news=x3['text']
category=x3['news_category']
from sklearn.model_selection import train_test_split
x_train1,x_test1,y_train1,y_test1=train_test_split(news,category,test_size=1/3,random_state=42)


from sklearn.feature_extraction.text import CountVectorizer
cv = CountVectorizer(binary = False, min_df = 5,max_df = 1.0, ngram_range=(1,2))
cv_train_features = cv.fit_transform(x_train1)
cv_test_features = cv.transform(x_test1)
...
lr.fit(cv_train_features,y_train1)
lr_predictions = lr.predict(cv_test_features )
lr_predictions


from sklearn.metrics import accuracy_score
accuracy_score(y_test1,lr_predictions)

from sklearn.metrics import confusion_matrix, classification_report

labels = ['negative','positive']
print(classification_report(y_test1,lr_predictions))


confusion_matrix(y_test1,lr_predictions)
...
classifier.fit(cv_train_features,y_train1)
cv_test_features=cv_test_features.toarray()
y_pred=classifier.predict(cv_test_features)
...
tv_train_features = tv.fit_transform(x_train1)
tv_test_features = tv.transform(x_test1)

lr.fit(tv_train_features,y_train1)  
...
from sklearn.naive_bayes import GaussianNB
classifier=GaussianNB()
tv_train_features=tv_train_features.toarray()
classifier.fit(tv_train_features,y_train1)
tv_test_features=tv_test_features.toarray()
y_pred=classifier.predict(tv_test_features)
...
##############################################################################################
###supervised and unsupervised sentiment  classification(mixed together) to predict sentiment
...
x_train,x_test,y_train,y_test=train_test_split(reviews,sentiment,test_size=1/3,random_state=42)
